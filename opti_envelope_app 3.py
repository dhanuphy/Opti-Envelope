...
class OpticalEnvelopeApp(QMainWindow):
    ...
    def run_simulation_for_thickness(self, d):
        ...
        Ti_unsafe = (2 * TM * Tm) / (TM + Tm)
        T_safe = np.clip(Ti_unsafe, 1e-6, 0.9999)
        d_cm = d * 1e-7
        alpha = (1 / d_cm) * np.log(1 / T_safe)
    ...
    def calculate_simple_parameters(self):
        ...
        try:
            ...
            d_cm = d * 1e-7
            T_safe = np.clip(self.transmittance, 1e-6, 0.9999)
            self.alpha_calc = -np.log(T_safe) / d_cm
    ...
    def calculate_all_parameters(self):
        ...
        if self.alpha_display is not None:
            ...
            # Avoid artificial skin-depth spike when alpha is zero or too small
            alpha_min = 100.0  # cm^-1, adjust if needed
            alpha_safe = np.array(self.alpha_display, dtype=float)
            alpha_safe[~np.isfinite(alpha_safe)] = np.nan
            alpha_safe[alpha_safe < alpha_min] = np.nan
            self.skin_depth = 1e7 / (2 * alpha_safe)
    ...
    def extract_optical_properties(self):
        ...
        try:
            ...
            # 1. Wavelength
            df["Lambda_nm"] = self.wavelength
            
            # 2. Measured Transmittance
            df["Transmittance"] = self.transmittance
            
            # 3. Upper envelope points (on envelope grid)
            if self.TM is not None and self.xx is not None:
                T_upper_interp = interp1d(self.xx, self.TM, 
                                         bounds_error=False, fill_value="extrapolate")(self.wavelength)
                wl_min = self.xx.min()
                wl_max = self.xx.max()
                T_upper_interp[(self.wavelength < wl_min) | (self.wavelength > wl_max)] = np.nan
                df["T_Upper_Envelope"] = T_upper_interp
            else:
                df["T_Upper_Envelope"] = np.nan
            
            # 4. Lower envelope points (on envelope grid)
            if self.Tm is not None and self.xx is not None:
                T_lower_interp = interp1d(self.xx, self.Tm, 
                                         bounds_error=False, fill_value="extrapolate")(self.wavelength)
                wl_min = self.xx.min()
                wl_max = self.xx.max()
                T_lower_interp[(self.wavelength < wl_min) | (self.wavelength > wl_max)] = np.nan
                df["T_Lower_Envelope"] = T_lower_interp
            else:
                df["T_Lower_Envelope"] = np.nan
            
            # 5. Simulated Transmittance
            if self.T_simulated is not None and self.xx is not None:
                T_sim_interp = interp1d(self.xx, self.T_simulated, 
                                       bounds_error=False, fill_value="extrapolate")(self.wavelength)
                ...
                df["T_Simulated"] = T_sim_interp
            else:
                df["T_Simulated"] = np.nan
            
            # 6. Interference-free Transmittance
            if self.interference_free_T is not None and self.xx is not None:
                T_if_interp = interp1d(self.xx, self.interference_free_T, 
                                      bounds_error=False, fill_value="extrapolate")(self.wavelength)
                wl_min = self.xx.min()
                ...
                df["T_Interference_Free"] = T_if_interp
            else:
                df["T_Interference_Free"] = np.nan
            
            # 7. n (Refractive index)
            df["n"] = self.n_final if self.n_final is not None else np.nan
            
            # 8. k (Extinction coefficient)
            df["k"] = self.k_final if self.k_final is not None else np.nan
            
            # 9. ε₁ (Epsilon 1)
            df["Epsilon1"] = self.e1 if self.e1 is not None else np.nan
            
            # 10. ε₂ (Epsilon 2)
            df["Epsilon2"] = self.e2 if self.e2 is not None else np.nan
            
            # 11. σ (Conductivity)
            df["Sigma_S_per_cm"] = self.sigma if self.sigma is not None else np.nan
            
            # 12. δ (Skin depth / tan delta - using tan_delta)
            df["Delta_tan_delta"] = self.tan_delta if self.tan_delta is not None else np.nan
            
            # 13. Skin Depth (nm)
            df["Skin_Depth_nm"] = self.skin_depth if self.skin_depth is not None else np.nan
            
            # 14. OD (Optical Density)
            df["OD_Optical_Density"] = self.optical_density if self.optical_density is not None else np.nan
            
            # 15. E (Energy in eV)
            df["E_eV"] = self.E if self.E is not None else 1240.0/self.wavelength
            
            # 16. α (Absorption coefficient)
            df["Alpha_cm_minus1"] = self.alpha_display if self.alpha_display is not None else np.nan
            
            # 17. αE (Alpha times Energy)
            df["Alpha_E"] = self.alphaE if self.alphaE is not None else np.nan
            
            # 18. (αE)² (Alpha times Energy squared)
            df["AlphaE_squared"] = self.alphaE_sq if self.alphaE_sq is not None else np.nan
            
            # 19. (αE)^0.5 (Alpha times Energy to power 0.5)
            df["AlphaE_sqrt"] = self.alphaE_sqrt if self.alphaE_sqrt is not None else np.nan
            
            # 20. dα/dE (Derivative of alpha with respect to energy)
            df["dAlpha_dE"] = self.d_alpha_dE if self.d_alpha_dE is not None else np.nan
            
            # 21. ln(α) (Natural log of alpha)
            df["ln_Alpha"] = self.ln_alpha if self.ln_alpha is not None else np.nan

            # Define export column order as specified by user
            column_order = [
                "Lambda_nm",
                "Transmittance",
                "T_Upper_Envelope",
